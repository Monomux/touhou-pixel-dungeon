...
package com.touhoupixel.touhoupixeldungeon.desktop;
	...
	public static void main (String[] args) {
		...
		if (SharedLibraryLoader.isWindows) {
			if (System.getProperties().getProperty("os.name").equals("Windows XP")) {
				basePath = "Application Data/.touhoupixel/Shattered Pixel Dungeon/";
			} else {
				basePath = "AppData/Roaming/.touhoupixel/Shattered Pixel Dungeon/";
			}
		} else if (SharedLibraryLoader.isMac) {
			basePath = "Library/Application Support/Shattered Pixel Dungeon/";
		} else if (SharedLibraryLoader.isLinux) {
			basePath = ".touhoupixel/shattered-pixel-dungeon/";
		}
		...
		FileHandle newPrefs = new Lwjgl3FileHandle(basePath + TPDSettings.DEFAULT_PREFS_FILE, Files.FileType.External);
		if (oldPrefs.exists() && !newPrefs.exists()){
			oldPrefs.copyTo(newPrefs);
		}

		config.setPreferencesConfig( basePath, Files.FileType.External );
		TPDSettings.set( new Lwjgl3Preferences( TPDSettings.DEFAULT_PREFS_FILE, basePath) );
		FileUtils.setDefaultFileProperties( Files.FileType.External, basePath );
		
		config.setWindowSizeLimits( 480, 320, -1, -1 );
		Point p = TPDSettings.windowResolution();
		config.setWindowedMode( p.x, p.y );

		config.setMaximized(TPDSettings.windowMaximized());

		if (TPDSettings.fullscreen()) {
		...
		new Lwjgl3Application(new TouhouPixelDungeon(new DesktopPlatformSupport()), config);
	}
}
