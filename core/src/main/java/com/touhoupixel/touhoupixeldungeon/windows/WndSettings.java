...
import com.touhoupixel.touhoupixeldungeon.ui.OptionSlider;
...
public class WndSettings extends WndTabbed {
	...
	public void hide() {
		...
		TouhouPixelDungeon.seamlessResetScene(new Game.SceneChangeCallback() {
			@Override
		...
		protected void createChildren() {
			...
			chkFullscreen = new CheckBox( Messages.get(this, "fullscreen") ) {
				...
				protected void onClick() {
					super.onClick();
					TPDSettings.fullscreen(checked());
				}
			};
			if (DeviceCompat.supportsFullScreen()){
				chkFullscreen.checked(TPDSettings.fullscreen());
			} else {
			...
			if ((int)Math.ceil(2* Game.density) < PixelScene.maxDefaultZoom) {
				optScale = new OptionSlider(Messages.get(this, "scale"),
					...
					protected void onChange() {
						if (getSelectedValue() != TPDSettings.scale()) {
							TPDSettings.scale(getSelectedValue());
							TouhouPixelDungeon.seamlessResetScene();
						}
					}
				};
			...
			if (DeviceCompat.isAndroid() && PixelScene.maxScreenZoom >= 2) {
				chkSaver = new CheckBox(Messages.get(this, "saver")) {
					...
					protected void onClick() {
						...
						if (checked()) {
							checked(!checked());
							TouhouPixelDungeon.scene().add(new WndOptions(Icons.get(Icons.DISPLAY),
								...
								protected void onSelect(int index) {
									if (index == 0) {
										checked(!checked());
										TPDSettings.powerSaver(checked());
									}
								}
							});
						} else {
				...
				chkSaver.checked( TPDSettings.powerSaver() );
				add( chkSaver );
			}

			if (DeviceCompat.isAndroid()) {
				Boolean landscape = TPDSettings.landscape();
				if (landscape == null){
				...
				btnOrientation = new RedButton(finalLandscape ?
					...
					@Override
					protected void onClick() {
						TPDSettings.landscape(!finalLandscape);
					}
				};
				add(btnOrientation);
			}
			...
			optBrightness = new OptionSlider(Messages.get(this, "brightness"),
				...
				@Override
				protected void onChange() {
					TPDSettings.brightness(getSelectedValue());
				}
			};
			optBrightness.setSelectedValue(TPDSettings.brightness());
			add(optBrightness);
			...
			optVisGrid = new OptionSlider(Messages.get(this, "visual_grid"),
				...
				@Override
				protected void onChange() {
					TPDSettings.visualGrid(getSelectedValue());
				}
			};
			optVisGrid.setSelectedValue(TPDSettings.visualGrid());
			add(optVisGrid);
	...
	private static class UITab extends Component {
		...
		protected void createChildren() {
			...
			float wMin = Game.width;
			float hMin = Game.height;
			if (Math.min(wMin, hMin) >= 2*Game.density){
				optUISize = new OptionSlider(
					...
					@Override
					protected void onChange() {
						TPDSettings.interfaceSize(getSelectedValue());
						TouhouPixelDungeon.seamlessResetScene();
					}
				};
				optUISize.setSelectedValue(TPDSettings.interfaceSize());
			}

			if (TPDSettings.interfaceSize() == 0) {
				...
				btnSplit = new RedButton(Messages.get(this, "split")) {
					...
					protected void onClick() {
						...
						TPDSettings.toolbarMode(Toolbar.Mode.SPLIT.name());
						Toolbar.updateLayout();
					}
				};
				if (TPDSettings.toolbarMode().equals(Toolbar.Mode.SPLIT.name()))
				...
				btnGrouped = new RedButton(Messages.get(this, "group")) {
					...
					protected void onClick() {
						...
						textColor(TITLE_COLOR);
						btnCentered.textColor(WHITE);
						TPDSettings.toolbarMode(Toolbar.Mode.GROUP.name());
						Toolbar.updateLayout();
					}
				};
				if (TPDSettings.toolbarMode().equals(Toolbar.Mode.GROUP.name()))
				...
				btnCentered = new RedButton(Messages.get(this, "center")) {
					...
					protected void onClick() {
						...
						btnGrouped.textColor(WHITE);
						textColor(TITLE_COLOR);
						TPDSettings.toolbarMode(Toolbar.Mode.CENTER.name());
						Toolbar.updateLayout();
					}
				};
				if (TPDSettings.toolbarMode().equals(Toolbar.Mode.CENTER.name()))
				...
				chkFlipToolbar = new CheckBox(Messages.get(this, "flip_toolbar")) {
					...
					protected void onClick() {
						super.onClick();
						TPDSettings.flipToolbar(checked());
						Toolbar.updateLayout();
					}
				};
				chkFlipToolbar.checked(TPDSettings.flipToolbar());
				add(chkFlipToolbar);
			}
			...
			chkFlipTags = new CheckBox(Messages.get(this, "flip_indicators")){
				...
				protected void onClick() {
					super.onClick();
					TPDSettings.flipTags(checked());
					GameScene.layoutTags();
				}
			};
			chkFlipTags.checked(TPDSettings.flipTags());
			add(chkFlipTags);
			...
			chkFont = new CheckBox(Messages.get(this, "system_font")){
				...
				protected void onClick() {
					...
					TouhouPixelDungeon.seamlessResetScene(new Game.SceneChangeCallback() {
						@Override
						public void beforeCreate() {
							TPDSettings.systemFont(checked());
						}
			...
			chkFont.checked(TPDSettings.systemFont());
			add(chkFont);
			...
			if (DeviceCompat.hasHardKeyboard()){
				...
				btnKeyBindings = new RedButton(Messages.get(this, "key_bindings")){
					...
					protected void onClick() {
						super.onClick();
						TouhouPixelDungeon.scene().addToFront(new WndKeyBindings());
					}
				};
	...
	private static class DataTab extends Component{
		...
		protected void createChildren() {
			...
			chkNews = new CheckBox(Messages.get(this, "news")){
				...
				protected void onClick() {
					super.onClick();
					TPDSettings.news(checked());
					News.clearArticles();
				}
			};
			chkNews.checked(TPDSettings.news());
			add(chkNews);
			...
			if (Updates.supportsUpdates() && Updates.isUpdateable()) {
				chkUpdates = new CheckBox(Messages.get(this, "updates")) {
					...
					protected void onClick() {
						super.onClick();
						TPDSettings.updates(checked());
						Updates.clearUpdate();
					}
				};
				chkUpdates.checked(TPDSettings.updates());
				add(chkUpdates);
				...
				if (Updates.supportsBetaChannel()){
					...
					chkBetas.checked(TPDSettings.betas());
					add(chkBetas);
				}
			}
			...
			if (!DeviceCompat.isDesktop()){
				chkWifi = new CheckBox(Messages.get(this, "wifi")){
					...
					protected void onClick() {
						super.onClick();
						TPDSettings.WiFi(checked());
					}
				};
				chkWifi.checked(TPDSettings.WiFi());
			}
		}
	...
	private static class AudioTab extends Component {
		...
		protected void createChildren() {
			...
			optMusic = new OptionSlider(Messages.get(this, "music_vol"), "0", "10", 0, 10) {
				@Override
				protected void onChange() {
					TPDSettings.musicVol(getSelectedValue());
				}
			};
			optMusic.setSelectedValue(TPDSettings.musicVol());
			add(optMusic);
			...
			chkMusicMute = new CheckBox(Messages.get(this, "music_mute")){
				...
				protected void onClick() {
					super.onClick();
					TPDSettings.music(!checked());
				}
			};
			chkMusicMute.checked(!TPDSettings.music());
			add(chkMusicMute);
			...
			optSFX = new OptionSlider(Messages.get(this, "sfx_vol"), "0", "10", 0, 10) {
				@Override
				protected void onChange() {
					TPDSettings.SFXVol(getSelectedValue());
					if (Random.Int(100) == 0){
			...
			optSFX.setSelectedValue(TPDSettings.SFXVol());
			add(optSFX);
			...
			chkMuteSFX = new CheckBox( Messages.get(this, "sfx_mute") ) {
				...
				protected void onClick() {
					super.onClick();
					TPDSettings.soundFx(!checked());
					Sample.INSTANCE.play( Assets.Sounds.CLICK );
				}
			};
			chkMuteSFX.checked(!TPDSettings.soundFx());
			add( chkMuteSFX );
			...
			if (DeviceCompat.isiOS() && Messages.lang() == Languages.ENGLISH){
				...
				chkIgnoreSilent = new CheckBox( Messages.get(this, "ignore_silent") ){
					...
					protected void onClick() {
						super.onClick();
						TPDSettings.ignoreSilentMode(checked());
					}
				};
				chkIgnoreSilent.checked(TPDSettings.ignoreSilentMode());
				add(chkIgnoreSilent);
			}
		}
				...
				RedButton btn = new RedButton(Messages.titleCase(langs.get(i).nativeName()), 8){
					...
					protected void onClick() {
						...
						TouhouPixelDungeon.seamlessResetScene(new Game.SceneChangeCallback() {
							@Override
							public void beforeCreate() {
								TPDSettings.language(langs.get(langIndex));
								GameLog.wipe();
								Game.platform.resetGenerators();
							}
							@Override
			...
			if (currLang != Languages.ENGLISH) {
				...
				btnCredits = new RedButton(credText, credText.length() > 9 ? 6 : 9) {
					...
					protected void onClick() {
						...
						TouhouPixelDungeon.scene().addToFront(credits);
					}
				};
				add(btnCredits);
			}
