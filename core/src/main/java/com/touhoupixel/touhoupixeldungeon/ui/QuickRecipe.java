...
import com.touhoupixel.touhoupixeldungeon.items.ArcaneResin;
...
public class QuickRecipe extends Component {
	...
	public QuickRecipe(Recipe r, ArrayList<Item> inputs, final Item output) {
		...
		for (final Item in : inputs) {
			...
			curr = new ItemSlot(in) {
				...
				protected void onClick() {
					TouhouPixelDungeon.scene().addToFront(new WndInfoItem(in));
				}
			};
		...
		this.output = new ItemSlot(output){
			@Override
			protected void onClick() {
				TouhouPixelDungeon.scene().addToFront(new WndInfoItem(output));
			}
		};
		if (!hasInputs){
	...
	public class arrow extends IconButton {
		...
		protected void onClick() {
			...
			((AlchemyScene) TouhouPixelDungeon.scene()).populate(ingredients, Dungeon.hero.belongings);
		}
	...
	public static ArrayList<QuickRecipe> getRecipes( int pageIdx ){
		...
		switch (pageIdx){
			...
			case 2:
				...
				result.add(null);
				result.add(new QuickRecipe(new Bloodcake.Recipe()));
				result.add(new QuickRecipe(new BloodWaffle.Recipe()));
				result.add(new QuickRecipe(new Halfapple.Recipe()));
				result.add(new QuickRecipe(new Halfcake.Recipe()));
				result.add(new QuickRecipe(new Iceapple.Recipe()));
				result.add(new QuickRecipe(new KnifeWaffle.Recipe()));
				result.add(new QuickRecipe(new PeachFruit.Recipe()));
				result.add(new QuickRecipe(new PeachWaffle.Recipe()));
				result.add(new QuickRecipe(new TenshiPeach.Recipe()));
				result.add(new QuickRecipe(new Watermelon.Recipe()));
