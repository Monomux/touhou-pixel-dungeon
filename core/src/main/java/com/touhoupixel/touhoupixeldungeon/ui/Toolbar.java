...
import com.touhoupixel.touhoupixeldungeon.windows.WndQuickBag;
...
public class Toolbar extends Component {
	...
	protected void layout() {
		...
		for(int i = 0; i <= 3; i++) {
			if (i == 0 && !TPDSettings.flipToolbar() ||
				i == 3 && TPDSettings.flipToolbar()){
				btnQuick[i].border(0, 2);
				btnQuick[i].frame(106, 0, 19, 24);
			} else if (i == 0 && TPDSettings.flipToolbar() ||
					i == 3 && !TPDSettings.flipToolbar()){
		...
		float right = width;
		switch(Mode.valueOf(TPDSettings.toolbarMode())){
			case SPLIT:
				btnWait.setPos(x, y);
