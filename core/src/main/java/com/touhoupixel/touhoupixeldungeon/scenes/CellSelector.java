...
public class CellSelector extends ScrollArea {
	...
	private GameAction heldAction = TPDAction.NONE;
	...
	private Signal.Listener<KeyEvent> keyListener = new Signal.Listener<KeyEvent>() {
		...
		public boolean onSignal(KeyEvent event) {
			...
			if (!event.pressed){
				...
				if (heldAction != TPDAction.NONE && heldAction == action) {
					resetKeyHold();
					return true;
				} else {
	...
	private boolean moveFromAction(GameAction action){
		...
		if (action == TPDAction.N)  cell += -Dungeon.level.width();
		if (action == TPDAction.NE) cell += +1-Dungeon.level.width();
		if (action == TPDAction.E)  cell += +1;
		if (action == TPDAction.SE) cell += +1+Dungeon.level.width();
		if (action == TPDAction.S)  cell += +Dungeon.level.width();
		if (action == TPDAction.SW) cell += -1+Dungeon.level.width();
		if (action == TPDAction.W)  cell += -1;
		if (action == TPDAction.NW) cell += -1-Dungeon.level.width();
