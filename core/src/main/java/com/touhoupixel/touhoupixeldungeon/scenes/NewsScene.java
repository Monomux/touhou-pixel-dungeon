...
import com.touhoupixel.touhoupixeldungeon.services.news.NewsArticle;
...
import com.touhoupixel.touhoupixeldungeon.windows.WndTitledMessage;
...
public class NewsScene extends PixelScene {
	...
	private static class NewsInfo extends Component {
		...
		protected void createChildren() {
			...
			if (!News.articlesAvailable()){
				if (TPDSettings.news()) {
					if (TPDSettings.WiFi() && !Game.platform.connectedToUnmeteredNetwork()) {
	...
	private static class ArticleButton extends StyledButton {
		...
		public ArticleButton(NewsArticle article) {
			...
			long lastRead = TPDSettings.newsLastRead();
		...
		protected void onClick() {
			...
			if (article.date.getTime() > TPDSettings.newsLastRead()){
				TPDSettings.newsLastRead(article.date.getTime());
			}
			TouhouPixelDungeon.scene().addToFront(new WndArticle(article));
		}
	}
