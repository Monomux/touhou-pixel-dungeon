...
import com.touhoupixel.touhoupixeldungeon.levels.LastShopLevel;
...
public class TerrainFeaturesTilemap extends DungeonTilemap {
	...
	protected int getTileVisual(int pos, int tile, boolean flat){
		...
		if (tile == Terrain.HIGH_GRASS){
			return 9 + (16*8)+16*stage + (DungeonTileSheet.tileVariance[pos] >= 50 ? 1 : 0);
		} else if (tile == Terrain.FURROWED_GRASS){
			return 11 + (16*8)+16*stage + (DungeonTileSheet.tileVariance[pos] >= 50 ? 1 : 0);
		} else if (tile == Terrain.GRASS) {
			return 13 + (16*8)+16*stage + (DungeonTileSheet.tileVariance[pos] >= 50 ? 1 : 0);
