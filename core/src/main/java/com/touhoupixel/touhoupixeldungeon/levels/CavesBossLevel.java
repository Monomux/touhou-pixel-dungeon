...
public class CavesBossLevel extends Level {
	...
	protected boolean build() {
		...
		for (int i= 14*width(); i < length(); i++) {
			if (map[i] == Terrain.EMPTY) {
				if (patch[i - 14*width()]){
					map[i] = Terrain.WATER;
				} else if (Random.Int(Dungeon.isChallenged(Challenges.CHAMPION_ENEMIES) ? 4 : 8) == 0){
	...
	public void seal() {
		...
		TenshiBoss boss = new TenshiBoss();
	...
	public void eliminatePylon(){
		...
		for (Mob m : mobs){
			if (m instanceof TenshiBoss){
				((TenshiBoss) m).loseSupercharge();
				PylonEnergy.energySourceSprite = m.sprite;
			} else if (m instanceof Pylon){
				pylonsRemaining++;
			}
		}
		int finalPylonsRemaining = Dungeon.isChallenged(Challenges.CHAMPION_ENEMIES) ? 1 : 2;
				...
				if (energySourceSprite == null){
					for (Char c : Actor.chars()){
						if (c instanceof Pylon && c.alignment != Char.Alignment.NEUTRAL){
							...
							break;
						} else if (c instanceof TenshiBoss){
							energySourceSprite = c.sprite;
						}
					}
					if (energySourceSprite == null){
