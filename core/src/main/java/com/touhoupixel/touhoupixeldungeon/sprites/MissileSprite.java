...
public class MissileSprite extends ItemSprite implements Tweener.Listener {
	...
	static {
		...
		ANGULAR_SPEEDS.put(SagumeSprite.ScorpioShot.class,   0);
		...
		ANGULAR_SPEEDS.put(SuikaSprite.TenguShuriken.class,      2160);
	}
	...
	private void setup( PointF from, PointF to, Item item, Callback listener ){
		...
		if (item instanceof Dart && Dungeon.hero.belongings.weapon() instanceof Crossbow){
			speed *= 3f;
				...
				|| item instanceof SagumeSprite.ScorpioShot
				|| item instanceof SuikaSprite.TenguShuriken){
			speed *= 1.5f;
		}
