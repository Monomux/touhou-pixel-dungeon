...
public class MegumuSprite extends MobSprite {
	...
	public MegumuSprite() {
		...
		spray.pour( SilenceParticle.FACTORY, 0.04f );
		spray.on = false;
	}
		...
		if (warnDist == 0){
			...
			for (int i = 0; i < Dungeon.level.length(); i++){
				if (ch.fieldOfView != null && ch.fieldOfView[i]
					...
					e.pour(SilenceParticle.FACTORY, 0.04f);
	...
	public static class SilenceParticle extends PixelParticle.Shrinking {

		public static final Factory FACTORY = new Factory() {
			@Override
			public void emit( Emitter emitter, int index, float x, float y ) {
				((SilenceParticle)emitter.recycle( SilenceParticle.class )).reset( x, y );
			}
		};

		public SilenceParticle() {
			super();
