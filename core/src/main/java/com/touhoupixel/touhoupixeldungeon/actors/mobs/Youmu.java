...
public class Youmu extends Mob {
	...
	public int damageRoll() {
		...
		} else return Random.NormalIntRange(18, 23);
	}
	...
	public void die(Object cause) {
		if (cause != Chasm.class && cause != GhoulLifeLink.class && !Dungeon.level.pit[pos]){
			Youmu nearby = GhoulLifeLink.searchForHost(this);
			if (nearby != null){
	...
	private class Sleeping extends Mob.Sleeping {
		...
		public boolean act( boolean enemyInFOV, boolean justAlerted ) {
			Youmu partner = (Youmu) Actor.findById( partnerID );
	...
	public static class GhoulLifeLink extends Buff{
		...
		public void set(int turns, Youmu ghoul){
			this.ghoul = ghoul;
			turnsToRevive = turns;
		}
		...
		public void fx(boolean on) {
			if (on && ghoul != null && ghoul.sprite == null){
				...
				((YoumuSprite)ghoul.sprite).crumple();
			}
		}
		...
		public void detach() {
			super.detach();
			Youmu newHost = searchForHost(ghoul);
			if (newHost != null){
				attachTo(newHost);
				timeToNow();
			} else {
		...
		public void restoreFromBundle(Bundle bundle) {
			super.restoreFromBundle(bundle);
			ghoul = (Youmu) bundle.get(GHOUL);
			turnsToRevive = bundle.getInt(LEFT);
		}

		public static Youmu searchForHost(Youmu dieing){

			for (Char ch : Actor.chars()){
				if (ch != dieing && ch instanceof Youmu && ch.alignment == dieing.alignment){
