...
public class ReisenMkII extends Mob {
	...
	{
		spriteClass = ReisenMkIISprite.class;
		...
		public boolean act( boolean enemyInFOV, boolean justAlerted ) {
			...
			if (leapPos != -1){
				...
				sprite.jump(pos, leapPos, new Callback() {
					...
					public void call() {
						...
						if (endPos != leapPos){
							Actor.addDelayed(new Pushing(ReisenMkII.this, leapPos, endPos), -1);
						}
			...
			if (enemyInFOV && !isCharmedBy( enemy ) && canAttack( enemy )) {
				...
				if (enemyInFOV) {
					...
					state = WANDERING;
					target = Dungeon.level.randomDestination( ReisenMkII.this );
					return true;
				}
				...
				if (leapCooldown <= 0 && enemyInFOV && !rooted
					...
					if (b.collisionPos == targetPos){
						...
						if (Dungeon.level.heroFOV[pos] || Dungeon.level.heroFOV[leapPos]){
							GLog.w(Messages.get(ReisenMkII.this, "leap"));
							sprite.parent.addToBack(new TargetedCell(leapPos, 0xFF0000));
							((ReisenMkIISprite)sprite).leapPrep( leapPos );
