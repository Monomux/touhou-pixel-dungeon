...
public class YuyukoBoss extends Mob {
	...
	private HashSet<Mob> getSubjects(){
		...
		for (Mob m : Dungeon.level.mobs){
			if (m.alignment == alignment && (m instanceof Youmu || m instanceof Meiling || m instanceof Ran || m instanceof Kasen)){
				subjects.add(m);
			}
		}
		return subjects;
		...
		if (phase == 1) {
			...
			if (HP <= (Dungeon.isChallenged(Challenges.STRONGER_BOSSES) ? 100 : 50)) {
				...
				for (Mob m : Dungeon.level.mobs.toArray(new Mob[0])) {
					if (m instanceof Youmu || m instanceof Meiling || m instanceof Ran || m instanceof Kasen) {
						m.die(null);
					}
				}
			}
	...
	public static class DKGhoul extends Youmu {
		{
			state = HUNTING;
		}
	...
	public static class DKMonk extends Meiling {
		{
			state = HUNTING;
		}
	}

	public static class DKWarlock extends Ran {
		{
			state = HUNTING;
		}
	}

	public static class DKGolem extends Kasen {
		{
			state = HUNTING;
		}
	}
