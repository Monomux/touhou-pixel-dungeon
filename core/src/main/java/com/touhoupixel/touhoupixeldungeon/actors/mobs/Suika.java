...
import com.touhoupixel.touhoupixeldungeon.effects.particles.SmokeParticle;
...
import com.touhoupixel.touhoupixeldungeon.sprites.SuikaSprite;
...
public class Suika extends Mob {
	...
	//Suika is immune to debuffs and damage when removed from the level
	@Override
	...
	public boolean isAlive() {
		return super.isAlive() || Dungeon.level.mobs.contains(this); //Suika has special death rules, see prisonbosslevel.progress()
	}
	...
	//*****************************************************************************************
	//***** Suika abilities. These are expressed in game logic as buffs, blobs, and items *****
	//*****************************************************************************************
			...
			if (abilityToUse != lastAbility || Random.Int(10) == 0){
				switch (abilityToUse){
					case BOMB_ABILITY : default:
						abilityUsed = throwBomb(Suika.this, enemy);
						//if Suika cannot use his bomb ability first, use fire instead.
						if (abilitiesUsed == 0 && !abilityUsed){
							abilityToUse = FIRE_ABILITY;
							abilityUsed = throwFire(Suika.this, enemy);
						}
						break;
					case FIRE_ABILITY:
						abilityUsed = throwFire(Suika.this, enemy);
					...
					case SHOCKER_ABILITY:
						abilityUsed = throwShocker(Suika.this, enemy);
						//if Suika cannot use his shocker ability second, use fire instead.
			...
			if (timer == 3) {
				...
				for (int cell = 0; cell < PathFinder.distance.length; cell++) {
					...
					if (PathFinder.distance[cell] < Integer.MAX_VALUE) {
						...
						if (ch != null && !(ch instanceof Suika)) {
	...
	public static boolean throwFire(final Char thrower, final Char target){
		...
		for (int i = 0; i < PathFinder.CIRCLE8.length; i++){
			if (aim.sourcePos+PathFinder.CIRCLE8[i] == aim.path.get(1)){
				...
				Buff.append(thrower, Suika.FireAbility.class).direction = i;
				...
				for (int i = area.left; i < area.right; i++){
					for (int j = area.top; j < area.bottom; j++){
						...
						if (cur[cell] > 0 && off[cell] == 0){
							...
							Char ch = Actor.findChar( cell );
							if (ch != null && !ch.isImmune(Fire.class) && !(ch instanceof Suika)) {
