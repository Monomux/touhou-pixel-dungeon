...
public class Kasen extends Mob {
	...
	public int damageRoll() {
		if (Dungeon.depth > 50) {
			return Random.NormalIntRange(46, 51);
		} else return Random.NormalIntRange(18, 22);
	}
	...
	protected boolean act() {
		...
		enemyTeleCooldown--;
		if (teleporting){
			((KasenSprite)sprite).teleParticles(false);
	...
	private class Wandering extends Mob.Wandering{
		...
		protected boolean continueWandering() {
			...
			if (target != -1 && getCloser( target )) {
				...
				((KasenSprite)sprite).teleParticles(true);
				teleporting = true;
				spend( 2*TICK );
			} else {
				target = Dungeon.level.randomDestination( Kasen.this );
				spend( TICK );
			}
