...
public class Komachi extends Mob {
	...
	public int damageRoll() {
		if (Dungeon.depth > 50) {
			return Random.NormalIntRange(78, 83);
		} else return Random.NormalIntRange(48, 53);
	}
	...
	public void rollToDropLoot() {
		...
		lootChance *= Math.pow(1/2f, Dungeon.LimitedDrops.KOMACHI_EQUIP.count);
	...
	protected boolean act() {
		...
		enemyTeleCooldown--;
		if (teleporting){
			((KomachiSprite)sprite).teleParticles(false);
	...
	private class Wandering extends Mob.Wandering{
		...
		protected boolean continueWandering() {
			...
			if (target != -1 && getCloser( target )) {
				...
				((KomachiSprite)sprite).teleParticles(true);
				teleporting = true;
				spend( 2*TICK );
			} else {
				target = Dungeon.level.randomDestination( Komachi.this );
				spend( TICK );
			}
