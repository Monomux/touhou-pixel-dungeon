...
public class AndroidPlatformSupport extends PlatformSupport {
	...
	public void updateDisplaySize(){
		if (TPDSettings.landscape() != null) {
			AndroidGame.instance.setRequestedOrientation( TPDSettings.landscape() ?
		...
		if (fullscreen && TPDSettings.landscape() != null
				&& (Game.dispWidth >= Game.dispHeight) != TPDSettings.landscape()){
		...
		if (Game.dispWidth < renderWidth*2 || Game.dispHeight < renderHeight*2)
			TPDSettings.put( TPDSettings.KEY_POWER_SAVER, true );
		
		if (TPDSettings.powerSaver() && fullscreen){
